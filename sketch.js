...
function showboat(){
  ...
  if(boats.length>0){
    if(boats[boats.length-1].body.position.x<width-200 ||boats[boats.length-1]=== undefined){
     

      position = [-60,-70,-80,-40]
      ran = random(position)
      boat= new Boat(width,height-60,180,180,ran)
      boats.push(boat)
    }
  ...
  for(var i=0; i<boats.length; i++){
    ...
    var boat = new Boat(width,height-60,180,180,-100)
    boats.push(boat)
  }
}
